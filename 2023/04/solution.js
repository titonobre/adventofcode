...
function parseInput(input) {
    return input
        ...
        .map(line => {
            ...
            const winningNumbers = winningNumbersRaw.trim().split(numbersSeparator).map(value => Number(value));
            const myNumbers = myNumbersRaw.trim().split(numbersSeparator).map(value => Number(value));
